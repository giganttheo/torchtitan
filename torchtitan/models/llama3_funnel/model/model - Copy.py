...
class TransformerBlock(nn.Module):
    ...
    def __init__(self, layer_id: int, model_args: TransformerModelArgs):
        ...
        self.pooling_offset = self.pool_size - 1
    ...
    def forward(
        ...
        if self.pool_size > 1:
            ...
            pool_x = rearrange(x, "... (s c) e -> ... s c e", c=self.pool_size).sum(dim=-2) / self.pool_var
            pool_x = torch.concatenate((x[..., :self.pooling_offset, :], pool_x), dim=-2) #No pooling for first elements
            pool_attn = self.attention(self.attention_norm(pool_x), freqs_cis)
            pool_ffn = self.feed_forward(self.ffn_norm(pool_x + pool_attn))
            
            pool_out = pool_attn + pool_ffn
            
            trunc_out = pool_out[..., :self.pooling_offset, :]
            pool_out = pool_out[..., self.pooling_offset:, :] * self.pool_var
            
            pool_out = rearrange(pool_out, "... (s c) e -> ... s c e", c=1).expand(-1, -1, self.pool_size, -1)
            pool_out = rearrange(pool_out, "... s c e -> ... (s c) e")
            
            pool_out = torch.concatenate((trunc_out, pool_out), dim=-2)[..., :x.shape[-2], :] #No pooling for first elements
